...
class PilotNetDataset(Dataset):
    ...
    def __init__(
        ...
        Note: create the folder if it does not exist.""".replace(
            " " * 8, ""
        )
        ...
        if visualize is True:
            ...
            inds = np.random.RandomState(seed=42).permutation(
                len(all_samples) // sequence
            )
        if train is True:
            self.ind_map = (
                inds[: int(len(all_samples) / sequence * 0.8)] * sequence
            )
        else:
            self.ind_map = (
                inds[-int(len(all_samples) / sequence * 0.2) :] * sequence
            )
    ...
    def __getitem__(self, index: int):
        ...
        for i in range(self.sequence):
            ...
            if (
                np.abs(float(gt)) < 1e-5
                and i != 0
                and i != len(self.samples) - 1
            ):
